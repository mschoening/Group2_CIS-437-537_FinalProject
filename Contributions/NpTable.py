...
import pandas as pd   # needed to support csv exports
...
#CSV export code
def build_table_dataframe(desired_sizes, results_by_lb, lbmode_order, metric_key):
    rows = []
    for size in desired_sizes:
        row = {"Flow Size": size2str([size])[0]}
        for lb in lbmode_order:
            if lb in results_by_lb:
...
def main():
    ...
    for k, v in map_key_to_id.items():
        ...
        # Export to png for paper. 
        png_filename = fig_dir + "/{}.png".format("AVG_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        plt.savefig(png_filename, dpi=300, bbox_inches='tight')

        plt.close()

        # Export CSV table
        df_avg = build_table_dataframe(desired_sizes, results_by_lb, lbmode_order, "avg")
        csv_filename = fig_dir + "/{}.csv".format("AVG_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        ...
        # Export to png for paper.
        png_filename = fig_dir + "/{}.png".format("P99_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        plt.savefig(png_filename, dpi=300, bbox_inches='tight')

        plt.close()

        # Export CSV table
        df_p99 = build_table_dataframe(desired_sizes, results_by_lb, lbmode_order, "p99")
        csv_filename = fig_dir + "/{}.csv".format("P99_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        df_p99.to_csv(csv_filename, index=False)
